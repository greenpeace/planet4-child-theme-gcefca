export const setupHeader = function ($) {
  ...
  $(document).on(
    ...
    function toggleNavDropdown(evt) {
      ...
      if (!target) {
        throw new Error(
          "Missing `data-bs-target` attribute: specify the container to be toggled"
        );
      }
      const toggleClass = $button.data("bs-toggle");
      if (!toggleClass) {
        throw new Error(
          "Missing `data-bs-toggle` attribute: specify the class to toggle"
        );
      }
      ...
      $(".navbar-search-toggle").attr(
        "data-ga-action",
        $(".navbar-search-toggle").attr("aria-expanded") === "false"
          ? "Open Search"
          : "Close Search"
      );
    }
